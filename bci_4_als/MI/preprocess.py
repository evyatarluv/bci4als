...
def filter_eeg_data(eeg):
    ...
    # Params
    low_pass = params['preprocess']['filter']['low_pass']
    high_pass = params['preprocess']['filter']['high_pass']
    notch = params['preprocess']['filter']['notch']
    sample_freq = params['data']['sample_freq']
...
def save_clean_eeg(eeg, time_stamps, subject_path):
    ...
    cleaned_eeg = pd.DataFrame(data=eeg, columns=params['preprocess']['channel_names'])

    # Output the cleaned EEG
    output_path = os.path.join(subject_path, params['data']['filenames']['EEG_clean'])
